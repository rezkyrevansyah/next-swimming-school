...
async function PageContent({ params }: PageProps) {
  ...
  const { data: invoice, error } = await supabase
    ...
    .select(`
      ...
      coaches(id, coach_profiles(full_name, phone, alamat, nomor_rekening, nama_bank)),
      branches(name),
      coach_invoice_items(
  ...
  const items = (Array.isArray(invoice.coach_invoice_items) ? invoice.coach_invoice_items : [])
  ...
  const periodLabel = new Date(
    Number(invoice.period_month.split("-")[0]),
    Number(invoice.period_month.split("-")[1]) - 1
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {periodLabel} · {invoiceData.branch_name}
          </p>
        </div>
      </div>
...
export default function OwnerInvoiceDetailPage({ params }: PageProps) {
  return (
